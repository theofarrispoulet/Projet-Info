...
Option_Fin () {
	...
	echo -e "\033[35mLE PROGRAMME S'EST PARFAITEMENT EXÉCUTÉ ! \033[0m"
	exit 0
}
...
Options_Tris () {
	...
	if [[ $test4 = "--tab" ]]; then #Si l'argument saisi est "--tab", alors exécuter la commande suivante.
		...
		./fichiers_en_C/code_Tab #On exécute le programme qui gère les tableaux de données.
		...
		./fichiers_en_C/code_ABR #On exécute le programme qui gère les ABR.
		...
		gcc -o code_AVL code_AVL.c -lm #On compile le programme qui gère les AVL.
		./fichiers_en_C/code_AVL #On exécute le programme qui gère les AVL.
	fi
	Option_Fin
}
